...
// void acctmgr::changekey (account_name _acct, string _new_key) {
//     eosio_assert(_new_key.length() == 53, "Length of publik key should be 53");

//     string pubkey_prefix("EOS");
//     auto result = mismatch(pubkey_prefix.begin(), pubkey_prefix.end(), _new_key.begin());
//     eosio_assert(result.first == pubkey_prefix.end(), "Public key should be prefix with EOS");
//     auto base58substr = _new_key.substr(pubkey_prefix.length());
...
//       /// @abi action
//       void updateauth(public_key newkey) {
//          vector<key_weight> keys = { key_weight{newkey, 1} };
//          auto auth = authority{1, keys, {}, {}};
//          auto update_auth_payload = make_tuple(_self, N(test), N(active), auth);
...
/// @abi action
void acctmgr::updateauth(account_name _acct, public_key _newkey)
{
    vector<key_weight> keys = {key_weight{_newkey, 1}};
    auto auth = authority{1, keys, {}, {}};
    auto update_auth_payload = make_tuple(_self, N(_acct), N(active), auth);

    print ("payload: ", update_auth_payload, "\n");
    action(
        permission_level{_self, N(active)},
        N(eosio),
        N(updateauth),
